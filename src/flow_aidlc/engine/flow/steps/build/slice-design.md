...
- `worklog/<TICKET-ID>/shape/slices.md` — the slice entry (id, scope, files, requirement refs).
- `worklog/<TICKET-ID>/shape/design.md` — component-level decisions from Shape.
...
2. **Draft `worklog/<TICKET-ID>/build/<slice-id>/design.md`** with these sections:
...
3. **Stop** if any ambiguity cannot be resolved from existing docs. Raise a question in
   `worklog/<TICKET-ID>/journal.md` and wait for a human answer before continuing.
4. Save the document. Proceed to `steps/build/code-plan.md`.

## Output

`worklog/<TICKET-ID>/build/<slice-id>/design.md` — populated, no `[Answer]:` placeholders remaining.
