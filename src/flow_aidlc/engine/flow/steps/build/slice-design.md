...
- `worklog/<PI-NNN>/shape/slices.md` — the slice entry (id, scope, files, requirement refs).
- `worklog/<PI-NNN>/shape/design.md` — component-level decisions from Shape.
...
2. **Draft `worklog/<PI-NNN>/build/<slice-id>/design.md`** with these sections:
...
3. **Stop** if any ambiguity cannot be resolved from existing docs. Raise a question in
   `worklog/<PI-NNN>/journal.md` and wait for a human answer before continuing.
4. Save the document. Proceed to `steps/build/code-plan.md`.

## Output

`worklog/<PI-NNN>/build/<slice-id>/design.md` — populated, no `[Answer]:` placeholders remaining.
