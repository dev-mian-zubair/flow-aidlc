...
- Approved design from `worklog/<PI-NNN>/shape/design.md`.
- Approved requirements from `worklog/<PI-NNN>/shape/requirements.md` (for requirement
  reference ids).
...
```bash
cp .flow/templates/slices.tmpl.md worklog/<PI-NNN>/shape/slices.md
```
...
```bash
python -m flow_aidlc.checks.traceability worklog/<PI-NNN> --write
```

This emits `worklog/<PI-NNN>/shape/traceability.md` (the requirement→slice
