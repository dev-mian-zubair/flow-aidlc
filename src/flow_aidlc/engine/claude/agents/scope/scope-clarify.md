...
- **Knowledge Map (invariants):** read `knowledge/map/README.md` (index) each run, then the relevant subsystem map(s), per `.flow/steps/shared/knowledge-map.md`. Maps hold invariants only; an unmapped area is an open question, never an invention.
...
No Write/Edit. You MAY read `knowledge/map/**` and `.flow/knowledge-map.yaml` and query the read-only code graph (`mcp__graphify`); do not read or modify source files. Use the configured tracker's MCP (per `steps/shared/tracker.md`) only to read tracker context if needed. Do not create a worklog entry — that is `steps/shared/kickoff.md`'s job, after a task id is assigned.
