...
- The code graph (`mcp__graphify`) for structure; the Knowledge Map (`docs/flow/knowledge/map/`) for invariants.

## Workflow — graph-first, then seed, then fallback

1. **Seed** from the `Area` label + Affected file(s)/module(s), and read the matching `docs/flow/knowledge/map/<subsystem>.md` (index `docs/flow/knowledge/map/README.md`) — those hold invariants, not structure.
...
`STATUS: DONE | BLOCKED`, plus the `docs/flow/worklog/<TICKET-ID>/shape/map-existing.md` path (file paths, contracts, callers/dependents with `file:line`, and the don't-change list). Hand off to `shape-requirements`.

## Least privilege

`mcp__graphify` for read-only structural queries; Read/Grep/Glob for scoped source reading + the fallback; Read of `docs/flow/knowledge/map/**` for invariants; **Write scoped to `docs/flow/worklog/<TICKET-ID>/` only**. Bound the map to the relevant surface — the ticket's touched symbols and their immediate callers, not the whole transitive closure.
