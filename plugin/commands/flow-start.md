...
Then, for a **confirmed** ticket id: read `.flow/playbook.md`, load `.flow/steps/shared/kickoff.md` first, scaffold `worklog/<TICKET-ID>/` as directed, announce the task once, then enter the Shape phase by loading `.flow/steps/shape/map-existing.md` (for brownfield work) and `.flow/steps/shape/research.md` (when the feature needs a new external dependency the stack lacks) as conditional pre-steps, followed by `.flow/steps/shape/requirements.md` (invoking `superpowers:brainstorming`), `.flow/steps/shape/design.md`, and `.flow/steps/shape/slicing.md` in sequence, pausing at each checkpoint stage (`research`, `requirements`, `design`) for `/flow-approve` before advancing.
