...
**Inputs:** task id (`<PI-NNN>`), the ticket intent + acceptance criteria, and the Knowledge Map (`knowledge/map/`) for integration context.
...
4. Write `worklog/<PI-NNN>/shape/research.md` from `.flow/templates/research.tmpl.md` — options, recommendation, trade-offs, governance screen, integration notes, open questions.
5. **CHECKPOINT** — stop and present the recommendation + governance screen. Wait for `/flow-approve` before `shape-requirements`.

**Least privilege:** Read + web research (`WebSearch`, `WebFetch`) only; **Write scoped to `worklog/<PI-NNN>/` only** — no source writes. May read `knowledge/map/**` for integration context. Do **not** adopt or install anything — you produce a recommendation; the adoption decision graduates to `knowledge/decisions/` at `shape-design`.
